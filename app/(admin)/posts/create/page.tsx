...
export default function CreatePostPage() {
  ...
  useEffect(() => {
    if (editorRef.current && !quillInstance.current) {
      // Setup Quill dengan tema 'snow' dan toolbar yang kita butuhkan
      quillInstance.current = new Quill(editorRef.current, {
        theme: "snow",
        placeholder: "Tuliskan cerita dan wawasan lengkap Anda di sini...",
        modules: {
          toolbar: [
            [{ header: [2, 3, 4, false] }], // Batasi heading agar tidak merusak H1 website
      ...
      // Dengarkan perubahan teks, lalu masukkan format HTML-nya ke dalam State formData
      quillInstance.current.on("text-change", () => {
        if (quillInstance.current) {
          const htmlContent = quillInstance.current.root.innerHTML;
          // Cek jika kosong (Quill biasanya menyisakan <p><br></p> saat kosong)
