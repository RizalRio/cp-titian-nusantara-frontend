...
export default function CategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.id) {
        await api.put(`/api/v1/admin/categories/${formData.id}`, {
          name: formData.name,
        });
      } else {
        await api.post("/api/v1/admin/categories", { name: formData.name });
      }
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Berikan nama kategori. URL (slug) akan menyesuaikan secara
              otomatis.
            </DialogDescription>
          </DialogHeader>
